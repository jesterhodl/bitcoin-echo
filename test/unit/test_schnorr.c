...
static void test_tagged_hash_basic(void)
{
    ...
    if (!all_zero) {
        tests_passed++;
        printf("  [PASS] Tagged hash produces output\n");
    } else {
        printf("  [FAIL] Tagged hash produces output\n");
    }
}
...
static void test_tagged_hash_empty_msg(void)
{
    ...
    tests_passed++;
    printf("  [PASS] Tagged hash with empty message\n");
}
...
static void test_xonly_pubkey_parse_valid(void)
{
    ...
    if (secp256k1_xonly_pubkey_parse(&p, xonly)) {
        ...
        if (secp256k1_point_is_valid(&p) && !secp256k1_point_is_infinity(&p)) {
            tests_passed++;
            printf("  [PASS] Parse valid x-only pubkey (G)\n");
        } else {
            printf("  [FAIL] Parse valid x-only pubkey - point invalid\n");
        }
    } else {
        printf("  [FAIL] Parse valid x-only pubkey\n");
    }
}
...
static void test_xonly_pubkey_parse_invalid(void)
{
    ...
    if (!secp256k1_xonly_pubkey_parse(&p, xonly)) {
        tests_passed++;
        printf("  [PASS] Reject x >= p\n");
    } else {
        printf("  [FAIL] Reject x >= p\n");
    }
}
...
static void test_xonly_pubkey_serialize(void)
{
    ...
    if (memcmp(xonly_in, xonly_out, 32) == 0) {
        tests_passed++;
        printf("  [PASS] x-only serialize roundtrip\n");
    } else {
        printf("  [FAIL] x-only serialize roundtrip\n");
    }
}
...
static void test_schnorr_vector_0(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 0: valid signature\n");
    } else {
        printf("  [FAIL] Vector 0: valid signature\n");
    }
}
...
static void test_schnorr_vector_1(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 1: valid signature\n");
    } else {
        printf("  [FAIL] Vector 1: valid signature\n");
    }
}
...
static void test_schnorr_vector_2(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 2: valid signature\n");
    } else {
        printf("  [FAIL] Vector 2: valid signature\n");
    }
}
...
static void test_schnorr_vector_3(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 3: msg not reduced\n");
    } else {
        printf("  [FAIL] Vector 3: msg not reduced\n");
    }
}
...
static void test_schnorr_vector_4(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 4: r with leading zeros\n");
    } else {
        printf("  [FAIL] Vector 4: r with leading zeros\n");
    }
}
...
static void test_schnorr_vector_5(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 5: reject pubkey not on curve\n");
    } else {
        printf("  [FAIL] Vector 5: reject pubkey not on curve\n");
    }
}
...
static void test_schnorr_vector_6(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 6: reject odd R.y\n");
    } else {
        printf("  [FAIL] Vector 6: reject odd R.y\n");
    }
}
...
static void test_schnorr_vector_7(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 7: reject negated message\n");
    } else {
        printf("  [FAIL] Vector 7: reject negated message\n");
    }
}
...
static void test_schnorr_vector_8(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 8: reject negated s\n");
    } else {
        printf("  [FAIL] Vector 8: reject negated s\n");
    }
}
...
static void test_schnorr_vector_12(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 12: reject r == p\n");
    } else {
        printf("  [FAIL] Vector 12: reject r == p\n");
    }
}
...
static void test_schnorr_vector_13(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 13: reject s == n\n");
    } else {
        printf("  [FAIL] Vector 13: reject s == n\n");
    }
}
...
static void test_schnorr_vector_14(void)
{
    ...
    if (!secp256k1_schnorr_verify(sig, msg, 32, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 14: reject pubkey >= p\n");
    } else {
        printf("  [FAIL] Vector 14: reject pubkey >= p\n");
    }
}
...
static void test_schnorr_vector_15(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, NULL, 0, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 15: empty message\n");
    } else {
        printf("  [FAIL] Vector 15: empty message\n");
    }
}
...
static void test_schnorr_vector_16(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 1, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 16: 1-byte message\n");
    } else {
        printf("  [FAIL] Vector 16: 1-byte message\n");
    }
}
...
static void test_schnorr_vector_17(void)
{
    ...
    if (secp256k1_schnorr_verify(sig, msg, 17, pubkey)) {
        tests_passed++;
        printf("  [PASS] Vector 17: 17-byte message\n");
    } else {
        printf("  [FAIL] Vector 17: 17-byte message\n");
    }
}
...
int main(void)
{
    printf("Bitcoin Echo — Schnorr Signature Tests (BIP-340)\n");
    printf("================================================\n\n");
    ...
    printf("\nBIP-340 Test Vectors (valid signatures):\n");
    ...
    printf("\nBIP-340 Test Vectors (invalid signatures):\n");
    ...
    printf("\nBIP-340 Test Vectors (variable message sizes):\n");
