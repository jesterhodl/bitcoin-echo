...
static void test_insert_and_lookup_by_hash(void) {
    ...
    /* Verify fields */
    ASSERT(memcmp(retrieved.hash.bytes, entry.hash.bytes, 32) == 0, "hash mismatch");
    ASSERT(retrieved.height == entry.height, "height mismatch");
    ASSERT(retrieved.status == entry.status, "status mismatch");
    ASSERT(memcmp(retrieved.chainwork.bytes, entry.chainwork.bytes, 32) == 0, "chainwork mismatch");
...
static void test_get_best_chain(void) {
    ...
    ASSERT(memcmp(best.hash.bytes, entry3.hash.bytes, 32) == 0, "wrong best block");
...
static void test_get_prev(void) {
    ...
    ASSERT(memcmp(prev.hash.bytes, entry1.hash.bytes, 32) == 0, "prev hash mismatch");
...
static void test_find_common_ancestor(void) {
    ...
    result = block_index_db_find_common_ancestor(&bdb, &a2.hash, &b1.hash, &ancestor);
    ASSERT(result == ECHO_OK, "failed to find common ancestor");
    ASSERT(memcmp(ancestor.hash.bytes, genesis.hash.bytes, 32) == 0, "ancestor should be genesis");
...
static void test_mark_best_chain(void) {
    ...
    ASSERT((retrieved.status & BLOCK_STATUS_VALID_CHAIN) != 0, "best chain flag not set");
...
static void test_unmark_best_chain(void) {
    ...
    result = block_index_db_lookup_by_hash(&bdb, &entry.hash, &retrieved);
    ASSERT(result == ECHO_OK, "failed to lookup");
    ASSERT((retrieved.status & BLOCK_STATUS_VALID_CHAIN) == 0, "best chain flag not cleared");
    ASSERT((retrieved.status & BLOCK_STATUS_VALID_HEADER) != 0, "other flags should remain");
...
static void test_get_chainwork(void) {
    ...
    ASSERT(memcmp(chainwork.bytes, entry.chainwork.bytes, 32) == 0, "chainwork mismatch");
...
static void test_get_chain_block(void) {
    ...
    entry2.status = BLOCK_STATUS_VALID_HEADER | BLOCK_STATUS_VALID_CHAIN;  /* On best chain */
    ...
    ASSERT(memcmp(retrieved.hash.bytes, entry2.hash.bytes, 32) == 0, "should get entry2");
...
static void test_persistence(void) {
    ...
    ASSERT(memcmp(retrieved.hash.bytes, entry.hash.bytes, 32) == 0, "hash not persisted");
