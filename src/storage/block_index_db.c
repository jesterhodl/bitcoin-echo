...
static echo_result_t create_schema(db_t *db) {
    ...
    result = db_exec(db, "CREATE INDEX IF NOT EXISTS idx_height ON blocks(height);");
    if (result != ECHO_OK) {
        return result;
    }

    /* Create chainwork index for chain comparison */
    result = db_exec(db, "CREATE INDEX IF NOT EXISTS idx_chainwork ON blocks(chainwork);");
    if (result != ECHO_OK) {
        return result;
    }
...
static echo_result_t prepare_statements(block_index_db_t *bdb) {
    ...
    result = db_prepare(&bdb->db,
        "SELECT hash, height, header, chainwork, status FROM blocks WHERE hash = ?",
        &bdb->lookup_hash_stmt);
    if (result != ECHO_OK) return result;

    /* Lookup by height */
    result = db_prepare(&bdb->db,
        "SELECT hash, height, header, chainwork, status FROM blocks WHERE height = ? LIMIT 1",
        &bdb->lookup_height_stmt);
    if (result != ECHO_OK) return result;

    /* Insert */
    result = db_prepare(&bdb->db,
        "INSERT INTO blocks (hash, height, header, chainwork, status) VALUES (?, ?, ?, ?, ?)",
        &bdb->insert_stmt);
    if (result != ECHO_OK) return result;

    /* Update status */
    result = db_prepare(&bdb->db,
        "UPDATE blocks SET status = ? WHERE hash = ?",
 ...
 */
static void populate_entry_from_row(db_stmt_t *stmt, block_index_entry_t *entry) {
