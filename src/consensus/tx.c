                                  ...
                                  tx_t *tx, size_t *offset)
{
    ...
    if (result != ECHO_OK) return result;
    *offset += consumed;
    ...
    for (i = 0; i < tx->input_count; i++) {
        ...
        *offset += 4;
        ...
        result = varint_read(data + *offset, data_len - *offset, &script_len, &consumed);
        if (result != ECHO_OK) return result;
        *offset += consumed;
                                   ...
                                   tx_t *tx, size_t *offset)
{
    ...
    for (i = 0; i < tx->input_count; i++) {
        ...
        result = varint_read(data + *offset, data_len - *offset, &item_count, &consumed);
        if (result != ECHO_OK) return result;
        *offset += consumed;
        ...
        for (j = 0; j < ws->count; j++) {
            ...
            result = varint_read(data + *offset, data_len - *offset, &item_len, &consumed);
            if (result != ECHO_OK) return result;
            *offset += consumed;
                       ...
                       tx_t *tx, size_t *consumed)
{
    ...
    if (offset + 2 <= data_len && data[offset] == SEGWIT_MARKER) {
        ...
        /* If marker is 0 but flag is not 0 or 1, it's just input_count=0 which is invalid */
    }
    ...
    /* For SegWit detection: if we thought it was legacy but input_count is 0, error */
                           ...
                           uint8_t *buf, size_t buf_len, size_t *written)
{
    ...
    /* Input count */
    result = varint_write(buf + offset, buf_len - offset, tx->input_count, &var_written);
    if (result != ECHO_OK) return result;
    offset += var_written;
    ...
    for (i = 0; i < tx->input_count; i++) {
        ...
        /* ScriptSig */
        result = varint_write(buf + offset, buf_len - offset, input->script_sig_len, &var_written);
        if (result != ECHO_OK) return result;
        offset += var_written;
    ...
    /* Output count */
    result = varint_write(buf + offset, buf_len - offset, tx->output_count, &var_written);
    if (result != ECHO_OK) return result;
    offset += var_written;
    ...
    for (i = 0; i < tx->output_count; i++) {
        ...
        /* ScriptPubKey */
        result = varint_write(buf + offset, buf_len - offset, output->script_pubkey_len, &var_written);
        if (result != ECHO_OK) return result;
        offset += var_written;
    ...
    if (has_witness) {
        for (i = 0; i < tx->input_count; i++) {
            ...
            result = varint_write(buf + offset, buf_len - offset, ws->count, &var_written);
            if (result != ECHO_OK) return result;
            offset += var_written;

            for (j = 0; j < ws->count; j++) {
                result = varint_write(buf + offset, buf_len - offset, ws->items[j].len, &var_written);
                if (result != ECHO_OK) return result;
                offset += var_written;
...
echo_bool_t tx_is_coinbase(const tx_t *tx)
{
    ...
    if (tx == NULL) return ECHO_FALSE;
    if (tx->input_count != 1) return ECHO_FALSE;
